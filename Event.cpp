...
void events::AbandonedPlanet(std::shared_ptr<Spaceship> selectedShip) {
    int posibility, i;
    int startOfRange = 0;
    int endOfRange = 2;
    posibility = RandomNumberGenerator(startOfRange, endOfRange);
    std::cout << "We've found a abandoned planet\n\n";

    if (posibility == 0) {
        std::cout << "We've found a treasure\n\n";
    ...
    if (dialogueOption == 1) {
        ...
        if (selectedShip->getFuel() >= 33) {
            ...
            if (fledChance < selectedShip->getEscapePossibility()) {
                ...
                std::cout << "We've failed to flee.Space pirates will come to haunt us!\n\n";
                selectedShip->setFuel(selectedShip->getFuel() - 33);
        ...
        events::SpacePirates(std::shared_ptr<Spaceship> (selectedShip));
    }
};